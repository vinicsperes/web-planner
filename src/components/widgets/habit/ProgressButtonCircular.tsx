...
export function ProgressButton({ onClick, todayProgress, goal }: ProgressButtonProps) {
    ...
    return (
        <button
            onClick={onClick}
            className="shrink-0 relative flex justify-center w-12 h-12 bg-gray-600 rounded-full items-center text-white text-lg focus:outline-none hover:bg-gray-500 z-10"
            aria-label="Mark as completed or add progress"
        >
            {/* Barra de Progresso Circular */}
            {Array.from({ length: goal }).map(() => (
